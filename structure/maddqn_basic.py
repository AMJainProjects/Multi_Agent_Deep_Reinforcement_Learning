...
class TimesNetAgent(DQNAgent):
    """
    DQN Agent using TimesNet for time series feature extraction.
    ...
    def __init__(self, state_dim, action_dim, hidden_dim=128, learning_rate=0.001,
        ...
        self.policy_net = TimesNet(state_dim[1], hidden_dim, action_dim).to(device)
        self.target_net = TimesNet(state_dim[1], hidden_dim, action_dim).to(device)
...
class MADDQN:
    ...
    def __init__(self, state_dim, action_dim, hidden_dim=128, learning_rate=0.001,
        ...
        # Initialize risk agent (TimesNet with Sharpe ratio reward)
        self.risk_agent = TimesNetAgent(state_dim, action_dim, hidden_dim, learning_rate,
                                       gamma, epsilon_start, epsilon_end, epsilon_decay,
                                       target_update, buffer_size, batch_size, device)
        
        # Initialize return agent (TimesNet with mid-term return reward)
        self.return_agent = TimesNetAgent(state_dim, action_dim, hidden_dim, learning_rate,
                                         gamma, epsilon_start, epsilon_end, epsilon_decay,
                                         target_update, buffer_size, batch_size, device)
